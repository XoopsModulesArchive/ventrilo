...
function aceweb_table_one_form($editing, $heading)
{
    ...
    print "
    <form action='database_table_LGAFilter.php' method='POST' enctype='application/x-www-form-urlencoded'>\n
    ...
    $field_list = aceweb_get_LGAFilter_fields();
...
function aceweb_get_table_one_result($table_one_key = null)
{
    ...
    $bci = aceweb_get_LGAFilter_fields();

    $sql = 'select ' . aceweb_to_string($bci) . ' from ' . $xoopsDB->prefix('aceweb_LGAFilter');

    if (null != $LGAFilter_key) {
        ...
        $sql .= " where LGAFilter_key = $LGAFilter_key";
    }
...
function aceweb_table_one_main_select()
{
    ...
    while (false !== ($values = $xoopsDB->fetchArray($result))) {
        ...
        print "<form action='database_table_LGAFilter.php' method='$method' enctype='application/x-www-form-urlencoded'>\n";
...
function aceweb_table_one_delete()
{
    ...
    $sql = 'delete from ' . $xoopsDB->prefix('aceweb_LGAFilter') . ' where LGAFilter_key = ' . "'" . $GLOBALS['xoopsDB']->escape($param_table_one_key) . "'";
    ...
    if (!$xoopsDB->query($sql)) {
        ...
        redirect_header('database_table_LGAFilter.php', 1, _AM_ACEWEB_OK_DB);
    }
...
function aceweb_table_one_update()
{
    ...
    $sql = 'REPLACE INTO  ' . $xoopsDB->prefix('aceweb_LGAFilter') . ' (' . aceweb_to_string($field_list) . ') VALUES (';
