...
function aceweb_table_one_form($editing, $heading)
{
    ...
    print "
    <form action='database_table_client.php' method='POST' enctype='application/x-www-form-urlencoded'>\n
    ...
    $field_list = aceweb_get_client_fields();
...
function aceweb_get_table_one_result($table_one_key = null)
{
    ...
    $bci = aceweb_get_client_fields();

    $sql = 'select ' . aceweb_to_string($bci) . ' from ' . $xoopsDB->prefix('aceweb_client');
    ...
    if (null != $client_key) {
        ...
        $sql .= " where client_key = $client_key";
    }
...
function aceweb_table_one_main_select()
{
    ...
    while (false !== ($values = $xoopsDB->fetchArray($result))) {
        ...
        print "<form action='database_table_client.php' method='$method' enctype='application/x-www-form-urlencoded'>\n";
...
function aceweb_table_one_delete()
{
    ...
    $sql = 'delete from ' . $xoopsDB->prefix('aceweb_client') . ' where client_key = ' . "'" . $GLOBALS['xoopsDB']->escape($param_table_one_key) . "'";
    ...
    if (!$xoopsDB->query($sql)) {
        ...
        redirect_header('database_table_client.php', 1, _AM_ACEWEB_OK_DB);
    }
...
function aceweb_table_one_update()
{
    ...
    $sql = 'REPLACE INTO  ' . $xoopsDB->prefix('aceweb_client') . ' (' . aceweb_to_string($field_list) . ') VALUES (';
