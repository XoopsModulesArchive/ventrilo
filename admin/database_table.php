...
//
function ventrilo_table_one_edit_form()
{
    ventrilo_table_one_form(true, _AM_VENTRILO_LABEL_TABLE_ONE_EDIT);

    ventrilo_table_one_del_form();
}

function ventrilo_table_one_add_form()
{
    ventrilo_table_one_form(false, _AM_VENTRILO_LABEL_TABLE_ONE_ADD);
}
...
// returns nothing
//
function ventrilo_table_one_form($editing, $heading)
{
    ...
    if ($editing) {
        [$result, $error] = ventrilo_get_table_one_result($param_table_one_key);

        if ($error) {
            printf(_AM_VENTRILO_FMT_ERROR, $error['msg'], $error['data']);
    ...
    $field_list = ventrilo_get_table_one_fields();
    ...
    print "<input type='hidden' name='op' value='edit_post'>\n
                ...
                <input type='submit' value='" . _AM_VENTRILO_BUT_SAVE . "'>\n
                </td></tr>\n
            </table>\n
        </td></tr>\n
    </table>\n
...
// returns nothing
//
function ventrilo_table_one_del_form()
{
    global $param_table_one_key;

    $heading = _AM_VENTRILO_LABEL_TABLE_ONE_DEL;
    ...
    print "<tr>
                ...
                <td class='even'>\n
                    ...
                    <input type='submit' value='" . _AM_VENTRILO_BUT_DELETE . "'>\n
                </td></tr>\n
            </table>\n
        </td></tr>\n
    </table>\n
...
//
function ventrilo_get_table_one_result($table_one_key = null)
{
    global $xoopsDB;

    $bci = ventrilo_get_table_one_fields();

    $sql = 'select ' . ventrilo_to_string($bci) . ' from ' . $xoopsDB->prefix('ventrilo_table_one');

    if (null != $table_one_key) {
        ...
        if (!is_numeric($table_one_key)) {
            $error = ['msg' => _AM_VENTRILO_ERR_table_one_BAD_KEY, 'data' => $sql];

            return [null, $error];
        }

        $sql .= " where table_one_key = $table_one_key";
    }
...
// returns nothing
//
function ventrilo_table_one_main_select()
{
    $method = 'POST'; // POST, except when debugging

    global $xoopsDB;

    [$result, $error] = ventrilo_get_table_one_result();
    ...
    print '<th>' . _AM_VENTRILO_LABEL_TABLE_ONE_EDIT2 . "</th>\n";
    ...
    while (false !== ($values = $xoopsDB->fetchArray($result))) {
        ...
        print "<input type='submit' value='" . _AM_VENTRILO_BUT_EDIT . "'>\n";
    ...
    if (!$are_there_any) {
        print "<tr><td class = 'even' colspan = '" . (count($field_list) + 1) . "'>" . _AM_VENTRILO_ERR_TABLE_ONE_NONE . '</td></tr>';
    }
...
{
    $p_title = _AM_VENTRILO_TABLE_ONE_TITLE;
    ...
    ventrilo_table_one_add_form();

    ventrilo_table_one_main_select();
...
//
function ventrilo_table_one_main_select_post()
{
    xoops_cp_header();
...
//
function ventrilo_table_one_delete()
{
    global $xoopsDB;

    global $param_table_one_key;

    $sql = 'delete from ' . $xoopsDB->prefix('ventrilo_table_one') . ' where table_one_key = ' . "'" . $GLOBALS['xoopsDB']->escape($param_table_one_key) . "'";
    ...
    if (!$xoopsDB->query($sql)) {
        ...
        ventrilo_show_sql_error(_AM_VENTRILO_ERR_DELETE_FAILED, $error, $sql);

        xoops_cp_footer();
    } else {
        redirect_header('database_table.php', 1, _AM_VENTRILO_OK_DB);
    }
...
function ventrilo_table_one_update()
{
    global $xoopsDB;
    ...
    $sql = 'REPLACE INTO  ' . $xoopsDB->prefix('ventrilo_table_one') . ' (' . ventrilo_to_string($field_list) . ') VALUES (';
    ...
    if (!$xoopsDB->query($sql)) {
        ...
        ventrilo_show_sql_error(_AM_VENTRILO_ERR_REPLACE_FAILED, $error, $sql);
...
switch ($param_op) {
    case 'main':
        ...
        break;
    case 'select_post':
        ventrilo_table_one_main_select_post();
        break;
    case 'edit_post':
        ventrilo_table_one_update();
        break;
    case 'add_post':
        ventrilo_table_one_add();
        break;
    case 'del_post':
        ventrilo_table_one_delete();
        break;
    default:
        xoops_cp_header();
